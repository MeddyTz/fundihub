...
class _FundiProfileScreenState extends State<FundiProfileScreen>
  ...
  void dispose() {
    ...
    // Wipe cached fundi reels so the next profile opened starts fresh.
    // Must be post-frame: dispose() runs during build, direct
    // context.read would throw 'deactivated widget' in debug mode.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
  ...
  Future<void> _deletePortfolioImage(String uid, String url) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title:   const Text('Delete Workdone Photo?'),
        content: const Text(
            'Are you sure you want to delete this workdone photo?'),
        actions: [
          TextButton(
        ...
        body: TabBarView(
          ...
          children: [
            ...
            _ProfileTab(
              ...
              // Callback keeps mounted + _tabController in parent State
              onWorkPhotos: () async {
                final result = await context
                    .push<String?>('/fundi/work-photos');
                if (result == 'workPhotos' && mounted) {
...
class _ProfileTab extends StatelessWidget {
  ...
  final VoidCallback? onWorkPhotos;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (uid == context.read<AuthProvider>().userModel?.uid) ...[
              ...
              child: Column(
                ...
                children: [
                  ...
                  _FundiTile(
                    ...
                    title: AppL10n.of(context).helpSupport,
                    sub: 'WhatsApp, Email & FAQ',
                    onTap: () => _showFundiHelp(context,
                        AppL10n.of(context)),
                  ),
                  const SizedBox(height: AppTheme.spaceSM),
                  _FundiTile(
                    icon: Icons.share_rounded,
                    title: AppL10n.of(context).shareApp,
                    sub: 'Invite friends to FundiHub',
                    onTap: () => Share.share(
                        '🔧 Find trusted fundis on FundiHub!\n'
                        'https://play.google.com/store/apps/'
                        'details?id=com.fundihub.app'),
                  ),
                  const SizedBox(height: AppTheme.spaceSM),
                  _FundiTile(
                    icon: Icons.star_rounded,
        ...
        child: Column(
          ...
          children: [
            ...
            TextButton.icon(
              onPressed: () async {
                final result =
                    await context.push<String?>('/fundi/work-photos');
                if (result == 'workPhotos' && context.mounted) {
                  // already on Work Photos tab, just rebuild
                }
              },
...
// ── Fundi profile settings helpers ───────────────────────────────────────────

void _showFundiHelp(BuildContext context, AppL10n l10n) {
  showModalBottomSheet(
    context: context,
    isScrollControlled: true,
    backgroundColor: Colors.transparent,
    builder: (_) => _FundiHelpSheet(l10n: l10n),
  );
}

void _showFundiAbout(BuildContext context) {
  showDialog(
    context: context,
    builder: (_) => AlertDialog(
      shape: RoundedRectangleBorder(
...
Future<void> _confirmFundiDelete(
...
// ── Fundi settings tile — same visual as client _Tile ─────────────────────────

class _FundiTile extends StatelessWidget {
  final IconData icon;
  final String title;
  final String? sub;
  final VoidCallback onTap;
...
// ── Fundi language tile — identical to client _LanguageTile ──────────────────

class _FundiLangTile extends StatelessWidget {
  const _FundiLangTile();
...
class _FLangBtn extends StatelessWidget {
  final String flag, label;
  final bool selected;
  final VoidCallback onTap;
  const _FLangBtn({required this.flag, required this.label,
...
// ── Fundi help bottom sheet ───────────────────────────────────────────────────

class _FundiHelpSheet extends StatelessWidget {
  final AppL10n l10n;
  const _FundiHelpSheet({required this.l10n});

  Future<void> _whatsapp(BuildContext ctx) async {
    final uri = Uri.parse(
        'https://wa.me/255754967156?text='
        '${Uri.encodeComponent('Hello FundiHub Support, I need help.')}');
    if (await canLaunchUrl(uri)) {
      launchUrl(uri, mode: LaunchMode.externalApplication);
    } else if (ctx.mounted) {
      ScaffoldMessenger.of(ctx).showSnackBar(
          const SnackBar(content: Text('WhatsApp: +255754967156')));
    }
  }
  ...
  Future<void> _email(BuildContext ctx) async {
    ...
    bool opened = false;
    try { opened = await launchUrl(uri); } catch (_) {}
    if (!opened && ctx.mounted) {
      await Clipboard.setData(
          const ClipboardData(text: 'tztech26@gmail.com'));
      ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
          content: Text('Email copied: tztech26@gmail.com')));
    }
  }
  ...
  Widget build(BuildContext context) {
    final faqs = [
      [l10n.faqQ1, l10n.faqA1], [l10n.faqQ2, l10n.faqA2],
      [l10n.faqQ3, l10n.faqA3], [l10n.faqQ4, l10n.faqA4],
      [l10n.faqQ5, l10n.faqA5], [l10n.faqQ6, l10n.faqA6],
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  Expanded(child: _HelpContactBtn(
                ...
                Text('FAQ', style: AppTextStyles.titleMedium
                    .copyWith(fontWeight: FontWeight.w700)),
                const SizedBox(height: AppTheme.spaceMD),
                ...faqs.map((qa) => _FaqItem(q: qa[0], a: qa[1])),
              ],
            ),
          ),
        ),
...
class _HelpContactBtn extends StatelessWidget {
  final IconData icon; final String label;
  final Color color; final VoidCallback onTap;
  const _HelpContactBtn({required this.icon, required this.label,
